...
const ObjectsTemplate = "`<el-tree :props=\"objectProps\" :load=\"loadNode\" :empty-text=\"'None'\" :default-expanded-keys=\"expandArray\" node-key=\"id\" lazy><span class=\"custom-tree-node\" slot-scope=\"{ node, data }\"><el-tooltip v-if=\"data.desc && data.desc !== ''\" class=\"item\" effect=\"dark\" :content=\"data.desc\" placement=\"right\"><span :style=\"data['deprecated'] ? 'text-decoration: line-through;' : ''\">{{ node.label }}</span></el-tooltip><span v-else :style=\"data['deprecated'] ? 'text-decoration: line-through;' : ''\">{{ node.label }}</span><span style=\"color: #aaa; margin-left: 16px; font-size: 14px;\">{{ data.desc }}</span></span></el-tree>`"

const ObjectsContent = `Vue.component("objects", {
    data () {
        return {
            objectId: '',
            objectIsArray: false,
